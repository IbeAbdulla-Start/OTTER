...
int main() {
	...
	if (loadScene) {
		...
		Guid EpaphTex = ResourceManager::CreateTexture("textures/Epaph.png");
		...
		MaterialInfo::Sptr EpaphMaterial = std::make_shared<MaterialInfo>();
		EpaphMaterial->Shader = scene->BaseShader;
		EpaphMaterial->Texture = ResourceManager::GetTexture(EpaphTex);
		EpaphMaterial->Shininess = 1.0f;
		scene->Materials[EpaphMaterial->GetGUID()] = EpaphMaterial;
		...
		RenderObject Epaph = RenderObject();
		Epaph.Position = glm::vec3(0.0f, -3.05f, 2.99f);
		Epaph.Mesh = ResourceManager::GetMesh(monkeyMesh);
		Epaph.Material = EpaphMaterial;
		Epaph.Rotation.z = -91.634f;
		Epaph.Scale = glm::vec3(0.89f, 1.03f, 0.99f);
		Epaph.Name = "Epaph";
		scene->Objects.push_back(Epaph);
	...
	RenderObject* Epaph = scene->FindObjectByName("Epaph");
	...
	while (!glfwWindowShouldClose(window)) {
		...
		if (isDebugWindowOpen) {
			...
			if (DrawSaveLoadImGui(scene, scenePath)) {
				...
				Epaph = scene->FindObjectByName("Epaph");
			}
			ImGui::Separator();
		}
