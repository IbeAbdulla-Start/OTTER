...
namespace Gameplay {
	...
	Material::Sptr Material::FromJson(const nlohmann::json& data) {
		...
		result->SpecularTexture = ResourceManager::Get<Texture2D>(Guid(data["specular_texture"]));
	...
	nlohmann::json Material::ToJson() const { 
		return {
			...
			{"specular_texture", SpecularTexture ? SpecularTexture->IResource::GetGUID().str() : "null" },
			{ "shininess", Shininess },
		};
	}
}